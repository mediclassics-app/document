...
- 콘텐츠에 들어가는 인라인 태그는 모두 `{% raw %}{{{% endraw %}...{% raw %}}}{% endraw %}` 기호로 작성합니다.
...
텍스트의 형태를 나타내는 태그는 `{% raw %}{{{% endraw %}sm/...{% raw %}}}{% endraw %}`처럼 소문자 2글자를 사용하며, `+`를 사용하여 조합할 수 있습니다.

- 작은 글자 (small) : {% raw %}{{{% endraw %}sm/當歸{% raw %}}}{% endraw %} → `<span class="size-sm">當歸</span> `
- 큰 글자 (large) : {% raw %}{{{% endraw %}lg/當歸{% raw %}}}{% endraw %} → `<span class="size-lg">當歸</span>`
- 양각 (positive / relief) : {% raw %}{{{% endraw %}ps/當歸{% raw %}}}{% endraw %} → `<span class="print-ps">當歸</span>`
- 음각 (negative / intaglio) : {% raw %}{{{% endraw %}ng/當歸{% raw %}}}{% endraw %} → `<span class="print-ng">當歸</span>`
- 작은 글자면서 양각 : {% raw %}{{{% endraw %}sm+ps/當歸{% raw %}}}{% endraw %} → `<span class="size-sm print-ps">當歸</span>`
- 작은 글자면서 저본에 포함된 주석 (annotation) : {% raw %}{{{% endraw %}an+sm/李曰...{% raw %}}}{% endraw %} → `<span class="size-sm annotation">李曰...</span>`

#### ② 교감기 및 역자주

역자주는 `{% raw %}{{{% endraw %}note/주석 대상 : 설명 ※출전{% raw %}}}{% endraw %}`처럼 자유롭게 작성하고, 교감기는 `{% raw %}{{{% endraw %}note/대상 = 교감글자1 ※출전 ☆ *교감글자2 ※출전{% raw %}}}{% endraw %}`처럼 작성합니다.
...
# abc.yaml 파일
- id: 12
  text: {% raw %}{{{% endraw %}형기(形氣)란 육체와 정신을 아우르는 표현이다.{% raw %}}}{% endraw %}
```
...
- badge : 처방명, 본초명, 경혈명 제목의 뱃지
- formular-none : 처방 스타일링 제외 (순수 제목으로 스타일링)
- h2-title : 태그와 상관없이 h2 태그와 동일하게 스타일링 
- h3-title : 태그와 상관없이 h3 태그와 동일하게 스타일링 
- h4-title : 태그와 상관없이 h4 태그와 동일하게 스타일링 
- h5-title : 태그와 상관없이 h5 태그와 동일하게 스타일링 
- h6-title : 태그와 상관없이 h6 태그와 동일하게 스타일링 
